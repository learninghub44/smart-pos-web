...
CREATE TABLE IF NOT EXISTS plans (
  id            VARCHAR(50) PRIMARY KEY,           -- 'starter' | 'business' | 'enterprise' | 'lifetime'
  name          VARCHAR(100) NOT NULL,
  price_monthly INTEGER NOT NULL DEFAULT 0,        -- KES, 0 for lifetime
  price_once    INTEGER NOT NULL DEFAULT 0,        -- KES one-time (lifetime only)
  max_branches  INTEGER NOT NULL DEFAULT 1,        -- -1 = unlimited
  max_users     INTEGER NOT NULL DEFAULT 3,        -- -1 = unlimited
  max_products  INTEGER NOT NULL DEFAULT 500,      -- -1 = unlimited
...
-- ─────────────────────────────────────────────
-- 2. TENANTS  (one row per business / customer)
-- ─────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS tenants (
  id              UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  business_name   VARCHAR(255) NOT NULL,
  slug            VARCHAR(100) UNIQUE NOT NULL,    -- used in URLs, subdomains later
...
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  billing_type        VARCHAR(20) NOT NULL DEFAULT 'monthly'   -- monthly | lifetime
    CHECK (billing_type IN ('monthly', 'lifetime')),
  amount_paid         INTEGER NOT NULL,                        -- KES
  paystack_ref        VARCHAR(255),
  paystack_sub_code   VARCHAR(255),                           -- for recurring
...
CREATE TABLE IF NOT EXISTS tenant_users (
  ...
  branch_id     UUID,                              -- FK resolved below
...
-- Now we can add the FK on tenant_users.branch_id
...
-- ─────────────────────────────────────────────
-- 17. SETTINGS  (per tenant key-value store)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- 19. SUPER ADMIN (platform-level, not tenants)
-- ─────────────────────────────────────────────
...
DO $$ DECLARE t TEXT;
BEGIN
  FOR t IN SELECT unnest(ARRAY['tenants','subscriptions','tenant_users','branches',
    'categories','brands','suppliers','products','customers','purchases','returns','settings']) LOOP
    EXECUTE format(
